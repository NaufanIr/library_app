...
Future<List<BorrowsData>> fetchBorrows() async {
  final response = await http.get("${API.showPeminjaman}");
...
Future<List<BorrowsData>> fetchBorrowById({@required String id}) async {
  var borrowID = {"id": id};
  final response =
  await http.post("${API.showPeminjamanByID}", body: json.encode(borrowID));
  List jsonResponse = json.decode(response.body);
  if(response.statusCode == 200){
    return jsonResponse.map((e) => BorrowsData.fromJson(e)).toList();
  }else{
...
Future<String> addBorrow({
  ...
  final response =
  await http.post("${API.addPeminjaman}", body: borrowsData.toJson());
  if (response.statusCode == 200) {
